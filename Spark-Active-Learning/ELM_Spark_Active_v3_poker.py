...
if __name__=="__main__":
    conf = SparkConf().setMaster("spark://master:7077 ").setAppName("ActiveLearn")# spark://master:7077 local
    ...
    select_num = 1000  #选择最小样例个数   iris选择10个  pokerhand 选择500
    feature_num =10  #数据集中属性个数 从0列到  iris4  pokerhand 10
    class_index =10 #类标属性在第几列   iris 4  pokerhand 11
    iter_num = 2
    ...
    test_data = split_data.sample(False, 0.01, 87)#从所有数据中随机采样  到做小的test_data
    ...
    train_data,unlabel_data= split_data.randomSplit([0.01,0.99],7)# train_data.first() [4.6, 3.1, 1.5, 0.2, 0]   unlabel_data.first() #[5.1, 3.5, 1.4, 0.2, 0]
    ...
    #     ###############  ELM 训练  #############
         ...
         select_result = unlabel_data.map(lambda x:(enry(x),x)).sortByKey(ascending=False).top(select_num)
