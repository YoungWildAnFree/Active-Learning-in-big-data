...
if __name__=="__main__":
    z, y = make_blobs(n_samples=500 ,centers=3, random_state=1)
    print z
    print y
    print type(z)
    print type(y)
    data = np.concatenate([z,np.transpose([y])],axis=1)
    print data
    add_array = []

    conf = SparkConf().setMaster("local[4]").setAppName("ActiveLearn")
    ...
    select_num = 30 #选择最小样例个数
    feature_num =2  #数据集中属性个数 从0列到
    class_index = 3-1 #类标属性在第几列

    frist_filter =0# 初步过滤的概率值 小frist_filter值得将被选出
    #读入整个数据
    ...
    split_data=all_data.map(lambda x:yuchuli_data(x))#.map(lambda x:[float_feature(x[0:4]),iris_label_transfor(x[4])])
    ...
    print add_array
    plt.scatter(z[:, 0], z[:, 1])
    plt.scatter([i[0] for i in add_array],[i[1] for i in add_array],c='r')
    plt.show()
