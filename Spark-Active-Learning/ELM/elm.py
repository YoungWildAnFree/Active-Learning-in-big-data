...
class ELMRegressor(BaseELM, RegressorMixin):
    ...
    def _fit_regression(self, y):
        ...
        if (self.regressor is None):
            self.coefs_ = safe_sparse_dot(pinv2(self.hidden_activations_), y)  #pinv2 计算广义逆矩阵 safe_sparse_dot做点积 #β=HT
        else:
...
class ELMClassifier(BaseELM, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        y_bin = self.binarizer_.fit_transform(y) #将Y中的类标转换为二元值 （-1,1）
    ...
    def predict(self, X):
        ...
        # class_predictions = self.binarizer_.inverse_transform(raw_predictions) #将预测的到类标（-1,1）转换为以前的类标格式

        return raw_predictions  #class_predictions
