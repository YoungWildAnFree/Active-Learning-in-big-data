...
class BaseRandomHiddenLayer(BaseEstimator, TransformerMixin):
    ...
    _internal_activation_funcs = dict()   # {'sine': np.sin,'tanh': np.tanh,'tribas': _tribas,'sigmoid': _sigmoid,'hardlim': _hardlim }
    ...
    def __init__(self, n_hidden=20, random_state=0, activation_func=None,
        ...
        self.activation_func = activation_func   #'tanh'、'sine'、'tribas'、'sigmoid'、‘hardlim'
        self.activation_args = activation_args

        self.components_ = dict()  #components_['weights'] 从正太分布随机生成的权值(样例的维数*隐含层节点个数)和
                                # components_['biases'] 偏置(隐含层节点数个)
        self.input_activations_ = None   # safe_sparse_dot(X, w)+b 的值
    ...
    def _compute_hidden_activations(self, X):   #transform中返回值调用该函数  激活函数g(wx+b) 矩阵  H是隐层节点的输出
        """Compute hidden activations given X"""

        self._compute_input_activations(X)   #计算wx+b 将值付给self.input_activations_

        acts = self.input_activations_     #safe_sparse_dot(X, w)+b 的值
    ...
    def fit(self, X, y=None):
        ...
        self
        """
        X = atleast2d_or_csr(X)  #将np.array 转换为稀疏矩阵CSR（按行压缩稀疏格式）
...
class SimpleRandomHiddenLayer(BaseRandomHiddenLayer):
    ...
    def _generate_components(self, X):  #生成节点的偏置和权值
        """Generate components of hidden layer given X"""
    ...
    def _compute_input_activations(self, X):   #计算wx+b
        """Compute input activations given X"""

        b = self.components_['biases']
        w = self.components_['weights']

        self.input_activations_ = safe_sparse_dot(X, w) #稀疏矩阵矩阵点积
