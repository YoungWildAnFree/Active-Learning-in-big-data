...
def enry(x):
    ...
    result = -sum(ruanzuida(x) * np.log(ruanzuida(x)))  # 计算熵
    return result


if __name__=="__main__":
    conf = SparkConf().setMaster("local").setAppName("ActiveLearn")#spark://master:7077   local
    ...
    #     ###############  ELM 训练  #############
         ...
         select_result = unlabel_data.map(lambda x:(enry(x),x)).takeOrdered(select_num,key=lambda (x,y):(-x,y))#.sortByKey(ascending=False).top(select_num)
